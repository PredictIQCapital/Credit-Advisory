...
QUESTION_EN: dict[tuple[str, str], tuple[str, str, str]] = {
    ...
    ("unternehmen", "nace_code"): ("Industry code (WZ 2008 / NACE)", "e.g. C25.62 or 25.62 -- shown on the trade registration and the Creditreform report", "Lenders classify by this code. If given, it determines the comparison sector; the selection above is then only a cross-check."),
    ...
    ("unternehmen", "sicherheiten_wert"): ("Value of free (unencumbered) collateral", "Only what does not already secure another lender", "A rejection despite sound numbers is often a collateral problem -- which public guarantee programmes can solve."),
    ("unternehmen", "sicherheiten_belastet"): ("Value of collateral already pledged", "Land charges, transfers by way of security, assignments in favour of existing lenders", "Pledged collateral shows how much security the existing loans tie up -- and whether there is room for a second-ranking charge."),
...
DOCUMENT_EN = {
    ...
    "jahresabschluesse": ("Annual accounts of the last 3 years", "Balance sheet, P&L and notes, one file per year; three years, at least two. The latest one is read automatically for the quick check.", "The basis of every credit decision and a mandatory part of any loan application.", ""),
    "kapitalflussrechnung": ("Cash flow statement", "Cash flow statement under DRS 21, if prepared -- not mandatory for small and medium-sized GmbHs under HGB.", "Shows where liquidity actually came from. Without it the bank derives cash flow from the balance sheet and P&L itself -- as we do.", ""),
    ...
    "sicherheitenaufstellung": ("List of obligations and collateral", "All existing obligations (loans, leases, guarantees) and all collateral, split into free and already pledged (land charges, transfers by way of security, assignments), with land register extracts, valuations or machinery lists.", "Only free collateral is available for a new loan. Decides whether there is a collateral gap and how large it is.", ""),
